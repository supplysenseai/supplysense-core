...
// Maps risk-chart bar names → scenario (mirrors inventory-analyzer risk_distribution)
function riskSegmentFilter(segment: string) {
  switch (segment) {
    case "Low":      return (i: AnalyzedSKU) => i.scenario === "HEALTHY";
    case "Watch":    return (i: AnalyzedSKU) => i.scenario === "WATCH";
    case "Elevated": return (i: AnalyzedSKU) => i.scenario === "SLOW";
    case "Critical": return (i: AnalyzedSKU) => i.scenario === "CRITICAL";
    case "Dead":     return (i: AnalyzedSKU) => i.scenario === "DEAD";
...
export function buildDrilldown(
  ...
  if (ctx.chart === "risk_distribution") {
    const items = allSkus.filter(riskSegmentFilter(ctx.segment));
    return {
